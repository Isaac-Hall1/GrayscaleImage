 ...
 * Author: Ben Jones and ***Isaac Hall***
 */
public class GrayscaleImage {
    ...
    public double getPixel(int x, int y){
    	int width = this.imageData[0].length;
    	int height = this.imageData.length;
    	if(x >= 0 && x <= width - 1 && y >= 0 && y <= height - 1) {
    		return this.imageData[y][x];
    	}
    	//throws an error is the width and height arent within the bounds of the image
    	throw new IllegalArgumentException("x and y are not within the images width/height");
    }
    ...
    public boolean equals(Object other){
        ...
        //returns false if demensions do not match
        if(otherImage.imageData.length != this.imageData.length && otherImage.imageData[0].length != this.imageData[0].length) {
        	return false;
        }
        //compares each individual pixel
        for(int i = 0; i < imageData.length; i++) {
        	for(int j = 0; j < imageData[0].length; j++) {
        		if(otherImage.getPixel(i,j) == this.getPixel(i,j)) {
        			return true;
        		}
        	}
        }
    ...
    public double averageBrightness(){
    	double average = 0;
    	//takes the value of every pixel and averages it
    	for(int i = 0; i < this.imageData.length; i++) {
        	for(int j = 0; j < this.imageData[0].length; j++) {
        		average += getPixel(j,i);
        	}
    	}
    	average = average/(imageData.length * imageData[0].length);
        return average;
    }
    ...
    public GrayscaleImage normalized(){
    	...
    	double multiplyer = average / 127;
    	//uniformly multiplies every value of imageData to create a new averageBrightness of 127
    	for(int i = 0; i < this.imageData.length; i++) {
        	for(int j = 0; j < this.imageData[0].length; j++) {
        		this.imageData[i][j] = getPixel(j,i) / multiplyer;
        	}
    	}
    ...
    public GrayscaleImage mirrored(){
    	...
    	//reverses the values of 2d array such that each rows values are reversed
    	for(int i = 0; i < this.imageData.length; i++) {
        	for(int j = 0; j < this.imageData[0].length; j++) {
        		newImageData[i][j] = this.imageData[i][(this.imageData[0].length - 1) - j];
        	}
    	}
    ...
    public GrayscaleImage cropped(int startRow, int startCol, int width, int height){
        /**
         * take original array, pull number from the startRow and startCol and then apply it to a new array
         * with a new width and height
         */
    	if(width <= 0 || height <= 0) {
    		throw new IllegalArgumentException("width and height are less than or equal to 0 which isn't possible");
    	}
    	else if((startRow < 0 || startCol < 0) || (startRow >= this.imageData.length || startCol > this.imageData[0].length)) {
    		throw new IllegalArgumentException("The row or col specified is outside of the bounds of the original array");
    	}
    	double[][] newImageData = new double[width][height];
    	for(int i = 0; i < newImageData.length; i++) {
        	for(int j = 0; j < newImageData[0].length; j++) {
        		newImageData[i][j] = this.imageData[i + startRow][j + startCol];
        	}
    	}
    ...
    public GrayscaleImage squarified(){
    	//initialize the ways to take off the sides of an image
        int difference = Math.abs(this.imageData.length - this.imageData[0].length);
        int width = this.imageData[0].length;
        int height = this.imageData.length;
        ...
        //determine what the smallest side length is
        if(width >= height) {
        	smallerDimension = height;
        	differenceLeft = difference;
        }
        else if(height > width) {
        	smallerDimension = width;
        	differenceTop = difference;
        }
        double[][] newImageData = new double[smallerDimension][smallerDimension];
        for(int i = 0; i < height - differenceTop; i++) {
        	for(int j = 0; j < width - differenceLeft; j++) {
        		//copy values of correct positions to the newImageData 2D array
        		newImageData[i][j] = this.imageData[i + differenceTop/2][j + differenceLeft/2];
        	}
        }
        return (new GrayscaleImage(newImageData));
    }
